...
| `Requests` | the Pengajuan screen | Things somebody wants the masjid to spend money on — `type` is `beli` (something we do not have) or `perbaikan` (something we have, broken). Not stock: a request is what we WANT, an item is what we OWN, and a catalog containing both would undo the thing this register exists to fix. A **purchase** becomes stock at exactly one moment, when it is marked `selesai`; a **repair** never becomes stock at all — it appends a `status_change` putting one unit back to `available`. `status` is `diajukan` / `selesai` / `ditolak` (rows written before repairs existed say `dibeli`, which still reads as `selesai`). `price` is per unit on a purchase and the whole quote on a repair; `itemId` is set only when it is a restock of something already in the catalog; `assetId` is the unit being repaired, or the lost unit a purchase replaces. |
